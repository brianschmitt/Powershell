...
function New-Chart
{
    ...
    # create chart object
    $global:Chart = New-Object  -TypeName System.Windows.Forms.DataVisualization.Charting.Chart
...
function Out-Form
{
    ...
    if ($scriptBlock -is [ScriptBlock])
    {
        ...
        $timer.add_Tick({
                ...
                {
                    Remove-Points
                    $ht | ForEach-Object  -Process { $global:Chart.Series['Data'].Points.AddXY($($_.Keys), $($_.Values)) }
                }
                else
                {$global:Chart.Series['Data'].Points.DataBindXY($ht.Keys, $ht.Values) }
